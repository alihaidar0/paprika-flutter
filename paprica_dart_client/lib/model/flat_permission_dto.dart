...
class FlatPermissionDto {
  ...
  static List<FlatPermissionDto> listFromJson(List<dynamic> json) {
    return json == null
        ? new List<FlatPermissionDto>()
        : json.map((value) => new FlatPermissionDto.fromJson(value)).toList();
  }

  static Map<String, FlatPermissionDto> mapFromJson(
      Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, FlatPermissionDto>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) =>
          map[key] = new FlatPermissionDto.fromJson(value));
    }
    return map;
  }
}

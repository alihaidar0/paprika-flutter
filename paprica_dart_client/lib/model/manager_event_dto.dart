...
class ManagerEventDto {
  ...
  static List<ManagerEventDto> listFromJson(List<dynamic> json) {
    return json == null
        ? new List<ManagerEventDto>()
        : json.map((value) => new ManagerEventDto.fromJson(value)).toList();
  }

  static Map<String, ManagerEventDto> mapFromJson(
      Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, ManagerEventDto>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) =>
          map[key] = new ManagerEventDto.fromJson(value));
    }
    return map;
  }
}

...
class NotificationInfoDto {
  ...
  NotificationInfoDto.fromJson(Map<String, dynamic> json) {
    ...
    creationTime =
        json['creationTime'] == null ? null : DateTime.parse(json['creationTime'].toString().split('.').first);
    state = json['state'];
  ...
  static Map<String, NotificationInfoDto> mapFromJson(Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, NotificationInfoDto>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) => map[key] = new NotificationInfoDto.fromJson(value));
    }
    return map;
  }
}

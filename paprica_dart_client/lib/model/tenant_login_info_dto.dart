...
class TenantLoginInfoDto {
  ...
  Map<String, dynamic> toJson() {
    return {'tenancyName': tenancyName, 'name': name, 'id': id};
  }

  static List<TenantLoginInfoDto> listFromJson(List<dynamic> json) {
    return json == null
        ? new List<TenantLoginInfoDto>()
        : json.map((value) => new TenantLoginInfoDto.fromJson(value)).toList();
  }

  static Map<String, TenantLoginInfoDto> mapFromJson(
      Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, TenantLoginInfoDto>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) =>
          map[key] = new TenantLoginInfoDto.fromJson(value));
    }
    return map;
  }
}

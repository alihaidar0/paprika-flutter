...
class RegisterInput {
  ...
  static List<RegisterInput> listFromJson(List<dynamic> json) {
    return json == null
        ? new List<RegisterInput>()
        : json.map((value) => new RegisterInput.fromJson(value)).toList();
  }

  static Map<String, RegisterInput> mapFromJson(
      Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, RegisterInput>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) =>
          map[key] = new RegisterInput.fromJson(value));
    }
    return map;
  }
}

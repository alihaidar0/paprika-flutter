...
class UserNotificationInfo {
  ...
  static List<UserNotificationInfo> listFromJson(List<dynamic> json) {
    return json == null ? new List<UserNotificationInfo>() : json.map((value) => new UserNotificationInfo.fromJson(value)).toList();
  }

  static Map<String, UserNotificationInfo> mapFromJson(Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, UserNotificationInfo>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) => map[key] = new UserNotificationInfo.fromJson(value));
    }
    return map;
  }
}

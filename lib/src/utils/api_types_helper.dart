...
class ApiTypesHelper {
  ...
  Future<InitializationDto> _checkInitialConfig(BuildContext context, [bool fetchFresh]) async {
    ...
    if (fetchFresh == null || fetchFresh == false) {
      typesHash = await SharedPreference.loadHashFromSharedPref(SharedPreference.types);
      countriesHash = await SharedPreference.loadHashFromSharedPref(SharedPreference.countries);
      citiesHash = await SharedPreference.loadHashFromSharedPref(SharedPreference.cities);
      regionsHash = await SharedPreference.loadHashFromSharedPref(SharedPreference.regions);
    }
    InitializationDto config = await api.apiServicesAppCustomerConfigurationGetInitialConfigurationsGet(
        typesHash: typesHash, countriesHash: countriesHash, citiesHash: citiesHash, regionsHash: regionsHash);
  ...
  Future<bool> _validateTypes(BuildContext context, InitializationDto config) async {
    ...
    if (config != null && config.types.restaurantTypes.isNotEmpty) {
      bool saved = await Future.wait([
        SharedPreference.saveHashToSharedPref(SharedPreference.types, config.typesHash),
        SharedPreference.saveStringToSharedPref(SharedPreference.types, json.encode(config.types)),
      ]).then((results) {
      ...
      String typesStr = await SharedPreference.loadStringFromSharedPref(SharedPreference.types);
  ...
  Future<bool> _validateCountries(BuildContext context, InitializationDto config) async {
    // Note: Code generated from for API client always initializes objects even if the value from the server is null
    // So check for one empty array in the configuration to know if the object holding this array was originally set to null
    if (config != null && config.countries.items.isNotEmpty) {
      bool saved = await Future.wait([
        SharedPreference.saveHashToSharedPref(SharedPreference.countries, config.countriesHash),
        SharedPreference.saveStringToSharedPref(SharedPreference.countries, json.encode(config.countries)),
      ]).then((results) {
        return results.reduce((cumulative, value) => cumulative && value);
      }).catchError((err) {
        return false;
      ...
      // Load type from the shared preferences
      String countriesStr = await SharedPreference.loadStringFromSharedPref(SharedPreference.countries);

      PagedResultDtoConfigurationCountryDto countries;
      try {
        // parse
        countries = PagedResultDtoConfigurationCountryDto.fromJson(json.decode(countriesStr));
        if (countries != null) {
          // save
          return true;
        } else {
  ...
  Future<bool> _validateCities(BuildContext context, InitializationDto config) async {
    // Note: Code generated from for API client always initializes objects even if the value from the server is null
    // So check for one empty array in the configuration to know if the object holding this array was originally set to null
    if (config != null && config.cities.items.isNotEmpty) {
      bool saved = await Future.wait([
        SharedPreference.saveHashToSharedPref(SharedPreference.cities, config.citiesHash),
        SharedPreference.saveStringToSharedPref(SharedPreference.cities, json.encode(config.cities)),
      ]).then((results) {
        return results.reduce((cumulative, value) => cumulative && value);
      }).catchError((err) {
        return false;
      ...
      // Load type from the shared preferences
      String citiesStr = await SharedPreference.loadStringFromSharedPref(SharedPreference.cities);

      PagedResultDtoConfigurationCityDto cities;
      try {
        // parse
        cities = PagedResultDtoConfigurationCityDto.fromJson(json.decode(citiesStr));
        if (cities != null) {
          // save
          return true;
        } else {
  ...
  Future<bool> _validateRegions(BuildContext context, InitializationDto config) async {
    // Note: Code generated from for API client always initializes objects even if the value from the server is null
    // So check for one empty array in the configuration to know if the object holding this array was originally set to null
    if (config != null && config.regions.items.isNotEmpty) {
      bool saved = await Future.wait([
        SharedPreference.saveHashToSharedPref(SharedPreference.regions, config.regionsHash),
        SharedPreference.saveStringToSharedPref(SharedPreference.regions, json.encode(config.regions)),
      ]).then((results) {
        return results.reduce((cumulative, value) => cumulative && value);
      }).catchError((err) {
        DefaultErrorHandler.handle(context, err);
      ...
      String regionsStr = await SharedPreference.loadStringFromSharedPref(SharedPreference.regions);

      PagedResultDtoConfigurationRegionDto regions;
      try {
        // parse
        regions = PagedResultDtoConfigurationRegionDto.fromJson(json.decode(regionsStr));
        if (regions != null) {
          // save
          return true;
        } else {
  ...
  String getPlaceName(int value) {
    return types.restaurantTypes.firstWhere((restaurant) => restaurant.value == value)?.name;
  }
  ...
  int _calculateOrValueForList(List<EnumValue> lookupList, List<String> values) {
  ...
  List<String> getTypeNamesListByValue(int value, Type type){
    ...
    while(index < typeEnumValues.length) {
      ...
      if(typeEnumValues.any((element) => element.value == enumValue)){
        var currentEnum = typeEnumValues.firstWhere((element) => element.value == enumValue);
  ...
  String getCommaSeparatedTypeNamesByValue(int value, Type type, { int maxNumber = 1, String separator = ' ' }){
  ...
  int _getValue(List<EnumValue> lookupList, String name) {
    ...
    EnumValue lookup = lookupList.where((enumValue) => enumValue.name == name).first;
  ...
  Future<CountriesTreeModel> getCountries() async {
    ...
    PagedResultDtoConfigurationRegionDto regions;
    try {
      countries = PagedResultDtoConfigurationCountryDto.fromJson(json.decode(await SharedPreference.loadStringFromSharedPref(SharedPreference.countries)));

      cities = PagedResultDtoConfigurationCityDto.fromJson(json.decode(await SharedPreference.loadStringFromSharedPref(SharedPreference.cities)));

      regions = PagedResultDtoConfigurationRegionDto.fromJson(json.decode(await SharedPreference.loadStringFromSharedPref(SharedPreference.regions)));
    } catch (e) {
      return null;
    }
    ...
    for (int countriesIterator = 0; countriesIterator < countries.items.length; countriesIterator++) {
      ...
      for (int citiesIterator = 0; citiesIterator < cities.items.length; citiesIterator++) {
        if (cities.items[citiesIterator].countryId == country.id) {
          ...
          for (int regionsIterator = 0; regionsIterator < regions.items.length; regionsIterator++) {
  ...
  Future<Map<String, dynamic>> getCountriesMap() async {
    ...
    for (int countriesIterator = 0; countriesIterator < countries.items.length; countriesIterator++) {
      ...
      for (int citiesIterator = 0; citiesIterator < cities.items.length; citiesIterator++) {
        if (cities.items[citiesIterator].countryId == countries.items[countriesIterator].id) {
          ConfigurationCityDto cityDto = cities.items[citiesIterator];
          tree[countryDto.name]["cities"][cityDto.name] = {"id": cityDto.id, "regions": {}};

          for (int regionsIterator = 0; regionsIterator < regions.items.length; regionsIterator++) {
            if (regions.items[regionsIterator].cityId == cities.items[citiesIterator].countryId) {
              ConfigurationRegionDto regionDto = regions.items[regionsIterator];
              tree[countryDto.name]["cities"][cityDto.name]["regions"][regionDto.name] = {"id": regionDto.id};
            }
          }
        }
      }
  ...
  void initProfile({bool isAuthorized, String customerName, String phoneNumber, bool isPhoneNumberConfirmed}) {
    this.isAuthorized = isAuthorized;
